        ...
        {products.map(product => (
          <div key={product.id} className="border p-4 rounded-lg w-60">
            <div className="relative">
              <img src={handleImage(product?.images[0])} alt={product.name} className="w-full h-32 object-cover mb-2" />
              <span className="absolute top-2 left-2 bg-red-500 text-white px-2 py-1 text-xs rounded">{product.discount}</span>
            </div>
            <h3 className="text-lg font-semibold">{product.name}</h3>
            <p className="text-red-500">{product.price} <span className="line-through">{product.oldPrice}</span></p>
          </div>
        ))}
      </Carousel>
      <div className="flex justify-center mt-4">
        <button className="px-4 py-2 bg-red-500 text-white rounded-lg">View All Products</button>
      </div>
    </section>
      </div>
    </div>
